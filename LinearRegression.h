...
void LinearRegressionStrategy_(string symbol, vector<Record> inputDatatrain, const vector<Record>& inputData, int x, int p, vector<Order>& orderStatistics, ofstream& cashflowFile, ofstream& orderStatisticsFile, ofstream& finalPNLFile, int date_pos, int date_pos_train, int date_pos_end_train){
    date_pos_end_train = date_pos_end_train - 1;
    vector<double> open;
    vector<double> high;
    vector<double> low;
    vector<double> close;
    ...
    // cout<<"Date pos : "<<date_pos<<" Date pos train : "<<date_pos_train<<endl;
    cout<<inputDatatrain[date_pos_train].date<<" "<<inputDatatrain[date_pos_end_train].date<<endl;

    for (size_t i = date_pos_train; i < date_pos_end_train; ++i)
    {
        vector<double> row;
        row.push_back(close[i - 1]);
        row.push_back(vwap[i - 1]);
